...
class CpuTimes {
    ...
    _validateNewSample(new_cpu_t) {
        ...
        // If no samples before then should be ok
        let latest_cpu_t = this._cput_samples[0] || new CpuT( new_cpu_t.timestamp - 1 );
        
        if( new_cpu_t.timestamp <= latest_cpu_t.timestamp ) {
            throw new Error(`
                LoadavgWindows internal module error.
                Hint - it looks like "CpuT.now()" returns older sample than before.                
                *Fix it on your own or ask author for help.
