...
class CpuTimes {
    ...
    /**
     * @param {number} minimum required sample age
    ...
    constructor( { min_sample_age, sampling_interval} ) {
        ...
        this._self_timer = new WeakDaemon(sampling_interval, this.update, this);
    }
    ...
    _validateNewSample(new_cpu_t) {
        ...
        // TODO consider remove? - this check seems to be redundant after concerning this in tests
        if( ! (new_cpu_t instanceof CpuT) ) {
            throw new Error(`Internal module error. If you are not able to fix it on your own contact author.`);
        }
        ...
        if( new_cpu_t.timestamp <= latest_cpu_t.timestamp ) {
            ...
            console.error(                
                ...
                Details:   Expecting younger sample than latest one.
                           
                           New: ${new_cpu_t.timestamp}
                           
                           Latest: ${latest_cpu_t.timestamp}
            `);
        }
    }
