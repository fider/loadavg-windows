...
async function test() {
    ...
    const TIME_1_MIN_MS  = 60000;
    const TIME_10_SEC_MS = 10000; // 9.999s
    const TIME_5_SEC_MS  =  5000;

    const CHECK_TIME = TIME_5_SEC_MS;// TODO Fider go back to 15 or 20 sec
    const GC_INTERVAL = TIME_1_MIN_MS;


    const log_path = `C:/Projects/loadavg_logs__${Date.now()}.csv`;
    fs.writeFileSync(log_path, `time s;-;loadavg 1min;loadavg 5min;loadavg 15min;-;rss;heapTotal;heapUsed;external\n`);

    // Check if GC exposed
    gc();

    function infiniteLoop() {
        let time_s = parseInt((Date.now() - start_time) / 1000);
        let minutes = parseInt(time_s/60);
        let seconds = time_s % 60;

        let loadavg = os.loadavg();
        
        console.log(`${loadavg}   Time from start: ${minutes}:${seconds}`);
        logData(time_s, loadavg);
    ...
    function logData(time_s, loadavg) {
        let m = process.memoryUsage();    
        let log_message = `${time_s};-;${loadavg[0]};${loadavg[1]};${loadavg[2]};-;${m.rss};${m.heapTotal};${m.heapUsed};${m.external}\n`;
    ...
    setInterval(infiniteLoop, CHECK_TIME);
    setInterval(() => {
        gc();
        logGC();
    }, GC_INTERVAL);
    ...
    function LOG(log_message) {    
        ...
        log_cache.unshift(log_message);
        
        try {
            
            for(let i = log_cache.length-1; i>=0; i--) {
                let line = log_cache[i];
                fs.appendFileSync(log_path, line);
                
                // Line saved so I can remove it
                log_cache.pop();
            }
            
        } catch(err) {        
            if(err.code == 'EBUSY') {
                console.log('Log file locked  (log stored in cache and will be put into file as soon as it will be unlocked)');
            } else {
                throw err;
            }
        }
